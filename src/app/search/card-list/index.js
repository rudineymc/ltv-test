...
export default class CardListTemplate {
  ...
  buildCard() {
    const cardsHtml = this.cardList.reduce((cards, c) => {
      ...
      $card.find(`${this.cardBaseClass}__description`).eq(0).text(description);
      $card.find(`${this.cardBaseClass}__name`).eq(0).text(`${first_name || ''} ${last_name || ''}`);
      $card.find(`${this.cardBaseClass}__email`).eq(0).text(email);
      $card.find(`${this.cardBaseClass}__address`).eq(0).text(address);
      $card.find(`${this.cardBaseClass}__phones`).eq(0).append('<ul>' + phones + '</ul>');
      $card.find(`${this.cardBaseClass}__relatives`).eq(0).append('<ul>' + relativesList + '</ul>');
